...
export class NeuralFramesModule extends Module {
  constructor(id) {
    super(id, 'NEURAL');
    this.mode = 'smooth_still'; // smooth_still | trippy_zoom | infinite_rotation
    this.intensity = 0.5;
    this.speed = 1.0;
  ...
  // NEURALFRAMES MODIFIERS: Applied to frame data before render
  transform(frame) {
    // These modifiers affect seed drift + camera params for blind continuity
    switch(this.mode) {
      case 'smooth_still':
        // TRIPPY SMOOTH STILL: micro-oscillations on seed + latent noise
      ...
      case 'trippy_zoom':
        // TRIPPY ZOOM: exponential zoom with sine wave modulation
      ...
      case 'infinite_rotation':
        // INFINITE ROTATION: continuous spin + radius pulse
    ...
    // Drive phase by BPM if available
    if (frame.bpm) {
